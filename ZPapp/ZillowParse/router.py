...
router = APIRouter(
    ...
    tags=['Характеристики']
    # название этого роутера для объединения роутеров в группу в swagger
)
...
async def fetch_and_process_data(
    ...
    # Результат записываем в список словарей
    list_res_dicts = []
    ...
    for home in response_text['props']:
        ...
        for item in data_WFM:
            dist_to_WFM = distance((home['latitude'], home['longitude']),
                                   (item[1], item[2])).miles
        ...
        for item in data_TJ:
            dist_to_TJ = distance((home['latitude'], home['longitude']),
                                  (item[1], item[2])).miles
